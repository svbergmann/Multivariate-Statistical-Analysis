...
```{r}
confusion_matrices_pendigit <- vector(mode = 'list', length = num_hidden_nodes_pendigit)
for (i in 1:num_hidden_nodes_pendigit) {
  predictions <- factor(predict(nnets_pendigit[[i]], newdata = pendigit_test, type = 'class'), levels = levels(pendigit_test$digit))
  confusion_matrices_pendigit[[i]] <- confusionMatrix(predictions, pendigit_test$digit)
}
```
```{r}
mscrs_pendigit <- vector(mode = 'list', length = num_hidden_nodes_pendigit)
for (i in 1:num_hidden_nodes_pendigit) {
  mscrs_pendigit[[i]] <- 1 - confusion_matrices_pendigit[[i]]$overall["Accuracy"]
}
```
...
```{r}
cat('The lowest misclassificationrate is on the neural network \n with', which.min(mscrs_pendigit), 'hidden nodes and a rate of', mscrs_pendigit[[which.min(mscrs_pendigit)]], '.')
```
```{r}
print(confusion_matrices_pendigit[[which.min(mscrs_pendigit)]])
```

From the cross-classification tables, which digits have the largest misclassification rates?

It seems like the 2 gets predicted as 1 fairly often (75) times.
...
```{r}
confusion_matrices_spambase <- vector(mode = 'list', length = num_iterations_spambase)
for (i in 1:num_iterations_spambase) {
  predictions <- factor(predict(svmfits_spambase[[i]], newdata = spambase_test, type = 'class'), levels = levels(spambase_test$class))
  confusion_matrices_spambase[[i]] <- confusionMatrix(predictions, spambase_test$class)
}
```
```{r}
mscrs_spambase <- vector(mode = 'list', length = num_iterations_spambase)
for (i in 1:num_iterations_spambase) {
  mscrs_spambase[[i]] <- 1 - confusion_matrices_spambase[[i]]$overall["Accuracy"]
}
```
...
```
```{r}
print(confusion_matrices_spambase[[which.min(mscrs_spambase)]])
```
