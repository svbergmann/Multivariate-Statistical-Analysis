...
Observations with large negative component scores $\xi_{i1}$ are shorter banknotes with longer diagonal
whereas observations with large positive component scores $\xi_{i1}$ are taller banknotes with a shorter diagonal.
...
Observations with large negative component scores $\xi_{i2}$ are shorter banknotes
whereas Observations with large positive component scores $\xi_{i2}$ are longer banknotes.
...
legend("bottomleft", legend = levels(banknote_type), col = seq_along(banknote_type), pch = 16)
```
...
Observations with large negative component scores $\xi_{i1}$ are turtles with smaller length,
width and height whereas Observations with large positive component scores $\xi_{i1}$ are turtles with larger length, width and height.
...
Observations with large negative component scores $\xi_{i2}$ are taller turtles
whereas observations with large positive component scores $\xi_{i2}$ are shorter turtles.
...
legend("topleft", legend = levels(turtle_sex), col = seq_along(turtle_sex), pch = 16)
```
